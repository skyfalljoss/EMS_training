  ...
  return (
    <>
      ...
      <div className="glass-card" style={{marginTop:12}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
          <div className="profile-header" style={{border:'none',padding:0,marginBottom:24}}>
            ...
            <div className="profile-info">
              <h1>{d.name}</h1>
              <div className="role">{d.code}<span className={`status-pill ${d.status}`}>{statusLabel[d.status] || d.status}</span></div>
        ...
        <div style={{marginBottom:24}}>
          ...
          <div className="info-grid">
            ...
            <div className="info-field"><label>Status</label><div className="value"><span className={`status-pill ${d.status}`}>{statusLabel[d.status] || d.status}</span></div></div>
          </div>
        </div>
      ...
      <DepartmentFormModal
        ...
        onSaved={() => {
          getDepartment(id).then(data => setDepartment(data || null))
        }}
      />
